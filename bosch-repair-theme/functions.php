<?php
/**
 * Bosch Repair Pro - Theme Functions
 *
...
define( 'BRP_PHONE_RAW', '18005550199' );
define( 'BRP_EMAIL', 'service@boschrepairpro.com' );
define( 'BRP_BRAND', 'Bosch' );
...
function brp_theme_setup() {
    ...
    register_nav_menus( array(
        'primary'    => __( 'Primary Navigation', 'bosch-repair-pro' ),
        'footer-1'   => __( 'Footer Column 1 – Services', 'bosch-repair-pro' ),
        'footer-2'   => __( 'Footer Column 2 – Cities', 'bosch-repair-pro' ),
        'footer-3'   => __( 'Footer Column 3 – Resources', 'bosch-repair-pro' ),
        'footer-legal' => __( 'Footer Legal Links', 'bosch-repair-pro' ),
    ) );
}
...
function brp_register_taxonomies() {
    ...
    register_taxonomy( 'appliance_type', array( 'error_code', 'guide', 'service', 'recall' ), array(
        'labels' => array(
...
function brp_create_error_code_pages() {
    $appliances = array(
        'dishwasher'   => 'Bosch Dishwasher Error Codes',
        'washer'       => 'Bosch Washer Error Codes',
        'dryer'        => 'Bosch Dryer Error Codes',
        'refrigerator' => 'Bosch Refrigerator Error Codes',
        'oven'         => 'Bosch Oven & Range Error Codes',
        'cooktop'      => 'Bosch Cooktop Error Codes',
        'microwave'    => 'Bosch Microwave Error Codes',
        'freezer'      => 'Bosch Freezer Error Codes',
    );
...
function brp_breadcrumbs() {
    ...
    if ( is_singular( 'service' ) ) {
        ...
        echo '<a href="' . get_post_type_archive_link( 'guide' ) . '">Guides</a><span> › </span>';
        echo '<span>' . get_the_title() . '</span>';
        ...
        <div class="container">
            <div class="appointment-inner">
                ...
                <div class="appointment-form-wrapper">
                    <p class="appointment-form-title">&#128197; Book Your Appointment</p>
                    <form id="brpAppointmentForm" class="brp-appt-form" novalidate>
                        <?php wp_nonce_field( 'brp_nonce', 'brp_form_nonce', true, true ); ?>
                        <input type="hidden" name="action" value="brp_book_appointment">
                        <input type="hidden" name="brp_source_page" value="<?php echo esc_url( get_permalink() ?: home_url( $_SERVER['REQUEST_URI'] ) ); ?>">

                        <div class="brp-form-row">
                            <label for="brp_name">Full Name <span aria-hidden="true">*</span></label>
                            <input type="text" id="brp_name" name="brp_name" placeholder="John Smith" required>
                        </div>

                        <div class="brp-form-row">
                            <label for="brp_phone">Phone Number <span aria-hidden="true">*</span></label>
                            <input type="tel" id="brp_phone" name="brp_phone" placeholder="(555) 000-0000" required>
                        </div>

                        <div class="brp-form-row">
                            <label for="brp_email">Email Address</label>
                            <input type="email" id="brp_email" name="brp_email" placeholder="you@example.com">
                        </div>

                        <div class="brp-form-row">
                            <label for="brp_appliance">Appliance Type</label>
                            <select id="brp_appliance" name="brp_appliance">
                        ...
                        <div class="brp-form-row">
                            <label for="brp_preferred_date">Preferred Date</label>
                            <input type="date" id="brp_preferred_date" name="brp_preferred_date" autocomplete="off">
                            <span class="brp-date-msg" id="brpDateMsg" role="alert"></span>
                        </div>

                        <div class="brp-form-row" id="brpTimeRow" hidden>
                            <label for="brp_preferred_time">Preferred Time</label>
                            <select id="brp_preferred_time" name="brp_preferred_time">
                                <option value="">— Select a time slot —</option>
                            </select>
                        </div>

                        <div class="brp-form-row">
                            <label for="brp_message">Describe the Problem <small>(optional)</small></label>
                            <textarea id="brp_message" name="brp_message" rows="3" placeholder="e.g. Dishwasher shows E09 error, not heating"></textarea>
                        </div>

                        <div class="brp-form-feedback" id="brpFormFeedback" role="alert" aria-live="polite"></div>

                        <button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;" id="brpSubmitBtn">
                            <span class="brp-btn-text">&#128197; Request Appointment</span>
                            <span class="brp-btn-loading" hidden>Sending&hellip;</span>
                        </button>
                    </form>
                </div>
            </div>
...
function brp_get_services() {
    return array(
        array(
            'slug'  => 'bosch-washer-repair',
            'title' => 'Bosch Washer Repair',
            'icon'  => '🫧',
            'desc'  => 'Front-load and top-load Bosch washing machine diagnostics and repair.',
            'image' => 'washer.jpg',
        ),
        array(
            'slug'  => 'bosch-dryer-repair',
            'title' => 'Bosch Dryer Repair',
            'icon'  => '🌀',
            'desc'  => 'Expert Bosch dryer repair — heat element, drum belt, sensors and more.',
            'image' => 'dryer.jpg',
        ),
        array(
            'slug'  => 'bosch-dishwasher-repair',
            'title' => 'Bosch Dishwasher Repair',
            'icon'  => '🍽️',
            'desc'  => 'Bosch dishwasher not draining, leaking, or running? We fix it fast.',
            'image' => 'dishwasher.jpg',
        ),
        array(
            'slug'  => 'bosch-refrigerator-repair',
            'title' => 'Bosch Refrigerator Repair',
            'icon'  => '🧊',
            'desc'  => 'Bosch fridge and freezer repair — cooling, ice maker, water dispenser.',
            'image' => 'refrigerator.jpg',
        ),
        array(
            'slug'  => 'bosch-oven-repair',
            'title' => 'Bosch Oven Repair',
            'icon'  => '🔥',
            'desc'  => 'Wall oven, range oven, and speed oven repairs by certified technicians.',
            'image' => 'oven.jpg',
        ),
        array(
            'slug'  => 'bosch-range-repair',
            'title' => 'Bosch Range Repair',
            'icon'  => '🍳',
            'desc'  => 'Gas and electric Bosch range repair — burners, igniters, control boards.',
            'image' => 'range.jpg',
        ),
        array(
            'slug'  => 'bosch-cooktop-repair',
            'title' => 'Bosch Cooktop Repair',
            'icon'  => '♨️',
            'desc'  => 'Induction, gas, and electric Bosch cooktop repair services.',
            'image' => 'cooktop.jpg',
        ),
        array(
            'slug'  => 'bosch-freezer-repair',
            'title' => 'Bosch Freezer Repair',
            'icon'  => '❄️',
            'desc'  => 'Chest and upright Bosch freezer not freezing? We restore it quickly.',
            'image' => 'freezer.jpg',
        ),
        array(
            'slug'  => 'bosch-ice-maker-repair',
            'title' => 'Bosch Ice Maker Repair',
            'icon'  => '🧊',
            'desc'  => 'Bosch ice maker not making ice or leaking water — same-day repair.',
            'image' => 'ice-maker.jpg',
        ),
        array(
            'slug'  => 'bosch-microwave-repair',
            'title' => 'Bosch Microwave Repair',
            'icon'  => '📡',
            'desc'  => 'Built-in and countertop Bosch microwave repair services.',
            'image' => 'microwave.jpg',
        ),
        array(
            'slug'  => 'bosch-wall-oven-repair',
            'title' => 'Bosch Wall Oven Repair',
            'icon'  => '🔲',
            'desc'  => 'Single and double Bosch wall oven repair — heating elements, thermostats.',
            'image' => 'wall-oven.jpg',
        ),
        array(
            'slug'  => 'bosch-speed-oven-repair',
            'title' => 'Bosch Speed Oven Repair',
            'icon'  => '⚡',
            'desc'  => 'Bosch combination speed oven (microwave + convection) repair experts.',
            'image' => 'speed-oven.jpg',
        ),
    );
}
...
function brp_get_faqs_for_appliance( $appliance ) {
    $all = array(
        'washer' => array(
            array( 'q' => 'Why is my Bosch washer not spinning?', 'a' => 'A Bosch washer that won\'t spin is most commonly caused by an unbalanced load, a worn drive belt, a failed lid switch, or a faulty motor control board. Our technicians can diagnose and fix the issue same-day.' ),
            array( 'q' => 'Why is my Bosch washer leaking water?', 'a' => 'Leaks usually come from a damaged door seal/gasket, loose hose connections, a cracked tub, or a faulty water inlet valve. We stock all Bosch factory-certified gaskets and seals.' ),
            array( 'q' => 'Why does my Bosch washing machine smell bad?', 'a' => 'Front-load washers can develop mold and mildew in the drum and door gasket. We offer cleaning service and can replace the gasket with genuine Bosch parts.' ),
            array( 'q' => 'How long does a Bosch washer repair take?', 'a' => 'Most Bosch washer repairs are completed in a single visit of 1–2 hours. We carry common parts so same-day repair is possible in most cases.' ),
            array( 'q' => 'Is it worth repairing a Bosch washer?', 'a' => 'Yes — Bosch appliances are built to last 15–20 years. If the appliance is under 10 years old and the repair cost is less than 50% of replacement, repair is almost always the smarter financial choice.' ),
        ),
        'dryer' => array(
            array( 'q' => 'Why is my Bosch dryer not heating?', 'a' => 'A Bosch dryer that runs but doesn\'t heat is typically caused by a blown thermal fuse, failed heating element, tripped circuit breaker, or defective thermostat. Our technicians carry these parts to fix it on the spot.' ),
            array( 'q' => 'Why is my Bosch dryer taking too long to dry?', 'a' => 'Slow drying is commonly caused by a clogged lint trap or vent duct, a weak heating element, or moisture sensor failure. Regular vent cleaning also significantly improves drying performance.' ),
            array( 'q' => 'My Bosch dryer is making a loud noise — what should I do?', 'a' => 'Squeaking or thumping noises are usually caused by worn drum bearings, a damaged drive belt, or foreign objects in the drum. Stop using the dryer and call us to prevent further damage.' ),
            array( 'q' => 'How much does Bosch dryer repair cost?', 'a' => 'Bosch dryer repairs typically range from $150–$350 depending on the issue. We provide an upfront quote before any work begins and all repairs include a 90-day labor warranty.' ),
            array( 'q' => 'Can you repair a heat pump Bosch dryer?', 'a' => 'Yes, our technicians are trained on all Bosch dryer models including heat pump condensation dryers. We use factory-certified Bosch parts.' ),
        ),
        'dishwasher' => array(
            array( 'q' => 'Why is my Bosch dishwasher not draining?', 'a' => 'Drainage issues are usually caused by a clogged filter, blocked drain hose, failed drain pump motor, or a faulty check valve. Our technicians can clean or replace the necessary components quickly.' ),
            array( 'q' => 'Why is my Bosch dishwasher leaking?', 'a' => 'Leaks typically stem from a damaged door gasket, faulty water inlet valve, cracked spray arm, or pump seal failure. We carry genuine Bosch seals and gaskets to fix the leak properly.' ),
            array( 'q' => 'My Bosch dishwasher isn\'t cleaning dishes properly — what\'s wrong?', 'a' => 'Poor cleaning performance can be due to clogged spray arms, a failing wash pump, incorrect water temperature, or dirty filters. We diagnose the root cause and restore full cleaning performance.' ),
            array( 'q' => 'Why does my Bosch dishwasher leave spots on glasses?', 'a' => 'Spotting is often caused by low rinse-aid levels, hard water, or a malfunctioning rinse-aid dispenser. We can adjust settings and replace the dispenser if needed.' ),
            array( 'q' => 'What does error code E15 mean on a Bosch dishwasher?', 'a' => 'Error code E15 on a Bosch dishwasher indicates water in the base pan, which triggers the anti-flood float switch. This requires immediate attention to prevent water damage. Call us for same-day service.' ),
        ),
        'refrigerator' => array(
            array( 'q' => 'Why is my Bosch refrigerator leaking water?', 'a' => 'The most common causes of leaks include a clogged defrost drain, a cracked drain pan, a faulty water inlet valve, or a loose water line connection. We can diagnose and repair any of these issues.' ),
            array( 'q' => 'Why is my Bosch refrigerator not cooling?', 'a' => 'Cooling failure can be caused by a dirty condenser coil, failed compressor, low refrigerant, faulty evaporator fan, or a stuck defrost system. Our technicians carry diagnostic tools for accurate identification.' ),
            array( 'q' => 'Why is my Bosch refrigerator making noise?', 'a' => 'Common noises include compressor hum, condenser fan rattling, or ice maker cycling. Unusual clicking or banging sounds can indicate a failing compressor or loose parts. Call us for a free diagnostic estimate.' ),
            array( 'q' => 'My Bosch fridge ice maker stopped working — what could it be?', 'a' => 'Ice maker problems are often caused by a faulty water inlet valve, iced-over fill tube, failed ice maker module, or a water filter that needs replacing. We repair and replace all Bosch ice maker components.' ),
            array( 'q' => 'How do I reset my Bosch refrigerator?', 'a' => 'To reset most Bosch refrigerator models, hold the "Alarm Off" and "Super Freeze" buttons simultaneously for 3 seconds. If the issue persists after a reset, contact our technicians for professional diagnostics.' ),
        ),
        'oven' => array(
            array( 'q' => 'Why is my Bosch oven not heating to the right temperature?', 'a' => 'Temperature inaccuracy is typically caused by a faulty temperature sensor, failed heating element, or a miscalibrated control board. We recalibrate or replace the faulty component to restore accuracy.' ),
            array( 'q' => 'Why is my Bosch oven not turning on?', 'a' => 'The oven may not turn on due to a tripped circuit breaker, a failed thermal fuse, faulty igniter (gas), or a defective electronic control board. We diagnose and fix the root cause.' ),
            array( 'q' => 'Why does my Bosch oven door not close properly?', 'a' => 'Door issues are usually caused by worn or broken door hinges, a damaged door gasket, or a bent door frame. Proper sealing is essential for efficient cooking — we repair or replace faulty hinges and seals.' ),
            array( 'q' => 'My Bosch oven is self-cleaning but now won\'t unlock — what should I do?', 'a' => 'A locked door after a self-clean cycle is caused by a failed door latch motor or a stuck thermal lock. Do not force the door open. Call us and we\'ll safely resolve the lock issue.' ),
            array( 'q' => 'How often should I have my Bosch oven serviced?', 'a' => 'We recommend a professional inspection every 2–3 years to clean igniters, check seals, and calibrate temperature sensors. Preventive maintenance extends the life of your oven significantly.' ),
        ),
        'default' => array(
            array( 'q' => 'Do you use genuine Bosch parts?', 'a' => 'Yes. We use only factory-certified Bosch replacement parts to ensure your appliance performs to manufacturer specifications and your warranty remains intact.' ),
            array( 'q' => 'How quickly can you come out for a Bosch appliance repair?', 'a' => 'We offer same-day and next-day appointments in all service areas. Call us or use the scheduling form on this page and we\'ll have a technician at your door within 24 hours.' ),
            array( 'q' => 'Do you offer a warranty on your repairs?', 'a' => 'Yes. All repairs come with a 90-day labor warranty. The parts warranty follows the manufacturer\'s guarantee, which is typically 1 year for genuine Bosch parts.' ),
            array( 'q' => 'Are your technicians certified?', 'a' => 'Our technicians are highly trained and certified appliance repair professionals. They undergo regular training on new Bosch models and diagnostic techniques.' ),
            array( 'q' => 'Is your repair service affiliated with Bosch?', 'a' => 'No — we are an independent appliance repair service specializing in Bosch products. We are not affiliated with or endorsed by BSH Home Appliances (Bosch\'s manufacturer). Bosch is a registered trademark used here for identification purposes only.' ),
        ),
    );
...
// PAGE TITLE
// ============================================================
function brp_the_archive_title( $title ) {
    if ( is_post_type_archive( 'error_code' ) ) return 'Bosch Appliance Error Codes';
    if ( is_post_type_archive( 'guide' ) ) return 'Bosch Appliance Repair Guides';
    if ( is_post_type_archive( 'recall' ) ) return 'Bosch Appliance Recalls';
    if ( is_post_type_archive( 'city' ) ) return 'Cities We Service';
    if ( is_post_type_archive( 'service' ) ) return 'Our Bosch Repair Services';
    return $title;
}
add_filter( 'get_the_archive_title', 'brp_the_archive_title' );
...
function brp_add_meta_boxes() {
    $screens = array( 'service', 'city', 'error_code', 'recall', 'guide' );
    foreach ( $screens as $screen ) {
        add_meta_box( 'brp_page_meta', 'Page Settings', 'brp_meta_box_html', $screen, 'side', 'high' );
    }
}
...
function brp_meta_box_html( $post ) {
    ...
    $fields = array(
        '_brp_appliance_type' => 'Appliance Type (washer, dryer, dishwasher, refrigerator, oven, range, cooktop, freezer, microwave)',
...
function brp_save_meta( $post_id ) {
    ...
    $fields = array( '_brp_appliance_type', '_brp_error_code', '_brp_seo_title', '_brp_meta_desc' );
...
function brp_output_meta_description() {
    global $post;
    if ( is_singular() && $post ) {
        $desc = get_post_meta( $post->ID, '_brp_meta_desc', true );
        if ( $desc ) {
...
add_action( 'wp_head', 'brp_output_meta_description', 2 );
...
add_action( 'init', function() {
    ...
    if ( get_transient( 'brp_ec_pages_checked' ) ) return;
    set_transient( 'brp_ec_pages_checked', 1, DAY_IN_SECONDS );
